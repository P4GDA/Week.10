...
```{r}
testis_only_genes <- read.table("~/Markdown_Files/Week10/testis_only_gene_info.txt", stringsAsFactors = FALSE, header=FALSE, sep="\t")
...
```{r}
library(ggplot2)
ggplot(testis_only_genes, aes(abs(distance.to.promoter))) + geom_histogram(binwidth=500)
```
...
```{r}
GTEx_data <- read.table("~/Markdown_Files/Week10/GTEx_expression_data.txt", stringsAsFactors = FALSE, header=TRUE, sep="\t")
GTEx_data
```
...
```{r}
testis_only_genes_expr <- select(testis_only_genes_expr, contains(c("gene", "Liver", "Kidney", "Skin", "Spleen", "Testis")))
testis_only_genes_expr
```
...
```{r}
ggplot(testis_only_genes_expr, aes(Testis, Liver)) + geom_point() + geom_abline() + labs(x="Testis Expression", y="Liver Expression")
```
...
```{r}
ggplot(testis_only_genes_expr, aes(log(Testis), log(Liver))) + geom_point() + geom_abline() + labs(x="Testis Expression", y="Liver Expression")
```
...
```{r}
ggplot(testis_only_genes_expr, aes(Testis, Spleen)) + geom_point() + geom_abline() + labs(x="Testis Expression", y="Spleen Expression")
```

Again, as it is difficult to see the bottom left corner, let's log both axes:

```{r}
ggplot(testis_only_genes_expr, aes(log(Testis), log(Spleen))) + geom_point() + geom_abline() + labs(x="Testis Expression (log)", y="Spleen Expression (log)")
```
...
- We used one sample from each tissue type. Because there is variance between individual samples, it is better to use multiple replicates to find DHSs that are common across the samples. By using only one sample for each tissue, it is more likely to find incosistencies with the gene expression data.
